...
list=pd.read_excel("test.xls", encoding = 'utf8') #sometimes some people would prefer encoding in 'cp1252'
...
#        self.max = max.df
    ...
    def histogram(self, name):
        df = self.df
        ax = df.plot.hist(grid=True,normed=True)
        ax.set_xlabel(l)
#        plt.savefig(folder_path+'/graph/'+feature+'_hist.png', bbox_inches='tight')
    ...
    def boxplot(self):
        df = self.df
#        ######still in dvpt
#        df.boxplot(column = name,  grid=True, )
...
titles = clean_file(list)
list = titles.replace_binary()
print list

#for name in list.columns:  
#    print name
#    column = Graph(list[name])
#    column.histogram(name)
...
worksheet_descriptif.write_string(15,0,"count of missing values", bold_title)
##b.percent of missing values
cell_format_percent = workbook_descriptive.add_format({'num_format': '0.00%'})
worksheet_descriptif.set_row(16, None, cell_format_percent)#set row in percent
worksheet_descriptif.write_string(16,0,"% of missing values", bold_title)

worksheet_descriptif.write_string(18,0,"All calculations are made without taking care of missing values!!!")

i=1
for col in df.columns:
    worksheet_descriptif.write_number(15,i,df[col].isnull().sum()) #number of missing values
    worksheet_descriptif.write_number(16,i, float(df[col].isnull().sum())/len(df[col]))  #% of missing values
    i+=1
